...
# -------- RESEGMENTATION ON WHOLE SLIDE BUT ONLY CELLPOSE-SAM -------- 
...
if __name__ == "__main__":
    ...
    print("Starting Whole-Slide Cellpose Segmentation (Skipping Proseg)...")

    # 1. Point the tool directly at the whole-slide Zarr (No cropping)
    reseg = Resegmentation_xenium(
        zarr_dir=original_zarr_dir,          
        zarr_name=original_zarr_name,             
        output="dapi_whole_slide.png",  
        factor_rescale=4,
        image_name="morphology_focus",   
        label_name="cell_labels_global",
        shape_name="cell_boundaries_global",
        point_name="transcripts"
    )

    reseg.check_gpu()
    
    # 2. Extract and format the whole-slide DAPI
    print("Preprocessing whole slide DAPI...")
    reseg.preprocess_image(channel_names=["DAPI"], channels_to_use=["DAPI"])
    
    # 3. Run Cellpose (It handles its own internal tiling via tile_overlap)
    print("Running Cellpose on the entire slide...")
    reseg.run_cellpose(
        model_type="cyto3",
        diameter=None,
        gpu=True,
    ...
    # 4. Generate Shapes, assign Transcripts, and Save (NO PROSEG)
    print("Saving global shapes and metadata...")
    # proseg_refinement=False is the magic switch that skips the Proseg algorithm
    reseg.update_spatialdata(proseg_refinement=False, run_pipeline=True)

    print("Whole-slide Global Geometry complete! Master shapes generated.")