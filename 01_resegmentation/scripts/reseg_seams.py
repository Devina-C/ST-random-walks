...
class Resegmentation_xenium:
    ...
    def __init__(self, zarr_dir: str,
        ...
        self.vdata = None
        self.output_folder = temp_crop_dir.replace("temp_crops", "updated_temp")
...
if __name__ == "__main__":
    ...
    # No global_x_offset / global_y_offset.
    # Shapes are saved in local crop coordinates, exactly like patches.
    # The stitcher (stitching_dbscan.py) handles global placement for both.
    ...
    reseg.label_name = "cell_labels_intermediate"
    reseg.shape_name = "cell_boundaries_intermediate"
    reseg.update_spatialdata(proseg_refinement=False, run_pipeline=True)

    print("Running PROSEG and saving final output...")
    reseg.label_name = "cell_labels_seam_final"
    reseg.shape_name = "cell_boundaries_seam_final"
    reseg.update_spatialdata(proseg_refinement=True, run_pipeline=False)

    print(f"Finished: {patch_name_str}")

# ── Move final zarr to organised output folder ────────────────────────────
    
    actual_output_folder = reseg.output_folder 
    final_zarr_src  = os.path.join(actual_output_folder, f"updated_{crop_name}")
    
    seam_output_dir = os.path.join("data", "patches_seams")
    final_zarr_dest = os.path.join(seam_output_dir, f"integrated_proseg_{patch_name_str}.zarr")
    
    os.makedirs(seam_output_dir, exist_ok=True)

    if os.path.exists(final_zarr_src):
        if os.path.exists(final_zarr_dest):
            shutil.rmtree(final_zarr_dest)
        # move and rename
        shutil.move(final_zarr_src, final_zarr_dest)
        print(f"SUCCESS: Saved to {final_zarr_dest}")
    else:
        print(f"ERROR: Could not find {final_zarr_src}")
        if os.path.exists(actual_output_folder):
            print(f"Contents of {actual_output_folder}: {os.listdir(actual_output_folder)}")

    # ── Offset Sidecar (Crucial for the Stitcher) ─────────────────────────────
    sidecar = os.path.join("data", "patches_seams", f"integrated_proseg_{patch_name_str}_offsets.json")
    with open(sidecar, "w") as f:
        json.dump({"x_start": x_start, "y_start": y_start}, f)
    print(f"Offsets saved: x={x_start:.0f}, y={y_start:.0f}")
    ...
    # Clean up the proseg intermediate outputs (CSV/checkpoints)
    proseg_out_dir = f"proseg_output_{crop_name}"
    if os.path.exists(proseg_out_dir):
        shutil.rmtree(proseg_out_dir)

    # Clean up the "updated_temp_crops_seams" folder if it's now empty
    if os.path.exists(actual_output_folder) and not os.listdir(actual_output_folder):
        os.rmdir(actual_output_folder)