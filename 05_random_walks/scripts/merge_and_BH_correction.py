...
==========================
Merge chunk outputs from permutation_test.py at the same RWR_R value,
estimate lambda per sender from the RWR L* field (radial decay), and
apply BH-FDR correction with effective sample size n_eff.

Change RWR_R below to match the permutation run. Input/output files
are r-tagged so multiple r values can coexist.
"""
...
# ── parameters ────────────────────────────────────────────────────────
RWR_R                  = 0.7      # MUST match the permutation_test.py run
FDR_Q                  = 0.05
...
def main():
    ...
    if not os.path.exists(GRAPH_CACHE):
        raise FileNotFoundError(
            ...
            "Run permutation_test.py first to build/cache it."
        )
