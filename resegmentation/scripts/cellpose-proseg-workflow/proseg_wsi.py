#!/usr/bin/env python3
"""
Whole-slide Proseg tiling 

Uses globally merged Cellpose pixel canvas (merged_masks.npy) as prior
...
def run_proseg_tile(tile, merged, pts_df, tile_idx, n_tiles):
    ...
    try:
        ...
        if count_col:
            ...
            counts_all = np.array(adata_tile.X.sum(axis=1)).flatten()
            gdf_kept = gdf_kept.reset_index(drop=True)
            orig_idx = np.where(keep_mask)[0]
            gdf_kept['transcript_count'] = counts_all.iloc[orig_idx].values
            ts(f"Loaded transcript counts from X: mean= {counts_all[orig_idx].mean():.1f}")
