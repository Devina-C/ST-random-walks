...
"""
Seam resegmentation - based on reseg.py (32-patch script).
Only computes seam flow fields - NO MASKS.

crop window is calculated differently than reseg.py:
  - original patches: each crop = one tile (col*patch_w : (col+1)*patch_w)
  - seam patches: each crop = a margin-wide strip centred on a tile boundary

The stitcher script handles global placement.
...
if __name__ == "__main__":
    ...
    # ── Run pipeline — identical to reseg.py ────────────────────
    ...
    # ── CLEANUP ──────────────────────────────────────────────────────────────
    print("\nInitiating post-processing clean up")
    
    # Clean up the original crop
    if os.path.exists(crop_path):
        shutil.rmtree(crop_path)

    # Clean up the temp_crops_seams folder 
