...
# -------- RESEGMENTATION ON 32 PATCHES -------- 

if __name__ == "__main__":

    original_zarr_name = "BC_prime.zarr"
    original_zarr_dir = "xenium_output"
    ...
    try:
        ...
        print("Warning: No index provided. Defaulting to patch 0.")
        patch_index = 0

    n_cols = 4
    n_rows = 8

    if patch_index >= (n_cols * n_rows):
        raise ValueError(f"Index {patch_index} is out of bounds.")

    # calculate which tile this job belongs to
    row_idx = patch_index // n_cols
    col_idx = patch_index % n_cols

    print(f"Processing patch {patch_index}: Row {row_idx}, Col {col_idx}")

    # calculate pixel dimensions for a single patch

    patch_w = WHOLE_ROI['width'] / n_cols
    patch_h = WHOLE_ROI['height'] / n_rows

    # calculate coordinates of patch
    x_start = WHOLE_ROI['xmin'] + (col_idx * patch_w)
    x_end = x_start + patch_w

    y_start = WHOLE_ROI['ymin'] + (row_idx * patch_h)
    y_end = y_start + patch_h

    print(f"Cropping to: X[{x_start}]")

    # naming of patches
    patch_name_str = f"patch_{patch_index:02d}"
    ...
    print(f"Saving temporary crop to {crop_path}...")
    sdata_roi.write(crop_path)

    # run resegmentation tool on this small crop
    reseg = Resegmentation_xenium(
        zarr_dir=temp_crop_dir,          
        zarr_name=crop_name,             
        output=f"dapi_{patch_name_str}.png",  
        factor_rescale=2,
        image_name="morphology_focus",   
        label_name="cell_labels_reseg",
        shape_name="cell_boundaries_reseg",
    ...
    # use a dummy channel name "DAPI" to select the first channel
    ...
    # first call - runs pipeline and saves intermediate cellpose
    ...
    # second call - skips pipeline, just writes final with proseg
    print("Running proseg and saving final output...")
    reseg.label_name = "cell_labels_reseg"
    reseg.shape_name = "cell_boundaries_reseg"
    reseg.update_spatialdata(proseg_refinement=True, run_pipeline=False)

    print(f"Finished processing {patch_name_str}")